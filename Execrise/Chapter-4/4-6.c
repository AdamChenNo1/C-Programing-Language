 ...
 * @LastEditors: Elon C
 * @LastEditTime: 2021-01-23 01:17:51
 * @FilePath: \C-Programing-Language\Execrise\Chapter-4\4-6.c
 */
#include <stdio.h>
...
#define SIN 300       /*标识找到sin函数*/
#define EXP 301       /*标识找到exp函数*/
#define POW 302       /*标识找到pow函数*/
#define VARIABLE 1000 /*标识找到一个变量*/
int getop(char[]);
...
/*-----------------------变量寄存器-----------------------*/
#define VAR_LENGTH 26   /*变量数组长度*/
double reg[VAR_LENGTH]; //变量寄存器
int regf[VAR_LENGTH];   //变量是否赋值的记录
/*函数existVariable：若定义了变量a，返回1，否则返回0*/
double existVariable(int c)
{
    return regf[c - 'a'];
}

/*函数getVariable：若定义了变量a，返回a的值，否则报错，并返回0.0*/
double getVariable(int c)
{
    ...
    {
        printf("error:variable %c doesn't exist\n", c);
        return 0.0;
    }
}
